...
router.get('/estadisticas', function(req, res) {
  ...
  .then(function(DiagnosticosPrematuros) {
    ...
    return Turnos.find({$and:[{createdAt:{$lte:new Date()}},{createdAt:{$gte:new Date()}}]}).count().exec();
  })
  .then(function(cantidadTurnosDelDia) {
      ...
      return Turnos.find({$and:[{createdAt:{$lte:hoy}},{createdAt:{$gte:priorDate}}]}).count().exec();
  })
