...
router.post('/login', validation, (req, res) => {
    User.findOne({username: req.body.username}).then((user) => {
        ...
        if (!user.validPassword(req.body.password)) {
            res.status(400).json({error: 'invalid password!', code: 'authentication_failed'});
            return;
        }

        if (user.habilitado === false) {
            res.status(400).json({error: 'this user not enable', code: 'user_not_enabled'});
            return;
        }
        ...
        res.status(400).json({error: 'error por catch', code: 'authentication_failed'});
    });
});
